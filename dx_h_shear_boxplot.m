...
for file_num = 1:size(file_names,1)
    ...
    for cell_num = 1:size(use_table,2)
        ...
        resutls(ind).dx = resutls(ind).shears ./ resutls(ind).Es;
...
for pa = [5 10 20]
   ...
   use =strcmp(num2str(pa), pas);
   
   xx = [xx pa];
   yy = [yy median(dxs(use))];
...
ylabel('Shear strain - tan(\theta) (-)')

plot(xx/5,yy,'Color',[0.4660, 0.6740, 0.1880],'LineWidth',2)
