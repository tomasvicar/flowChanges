...
for ind = 1:length(resutls)
    ...
    for pa = [5 10 20]
        ...
        if any(use)
            ...
            hss = [hss,median(hs(use))];
            shearss = [shearss,median(shears(use))];
        end
...
for pa = [5 10 20]
    ...
    dx_h = dxs(use)./hss(use);
    
%     mean_shear = [mean_shear,mean(s)];
%     mean_dx_h = [mean_dx_h,mean(dx_h)];
%     std_shear = [std_shear,std(s)];
%     std_dx_h = [std_dx_h,std(dx_h)];
%     ste_shear = [ste_shear,std(s)/sqrt(sum(use))];
%     ste_dx_h = [ste_dx_h,std(dx_h)/sqrt(sum(use))];

    
        median_shear = [median_shear,median(s)];
        median_dx_h = [median_dx_h,median(dx_h)];
        q1_dx_h = [q1_dx_h,quantile(dx_h,0.25)];
        q3_dx_h = [q3_dx_h,quantile(dx_h,0.75)];
        
        
    
    
end
median_shear = [5,10,20];
mean_shear = [5,10,20];
...
median_shear = [0,median_shear];

median_dx_h = [0,median_dx_h];
q1_dx_h = [0,q1_dx_h];
q3_dx_h = [0,q3_dx_h];

% mean_shear = [0,mean_shear];
% mean_dx_h = [0,mean_dx_h];
% ste_dx_h = [0,ste_dx_h];
...
% errorbar(mean_shear,mean_dx_h,ste_dx_h,ste_dx_h)
errorbar(median_shear,median_dx_h,q1_dx_h,q3_dx_h)
