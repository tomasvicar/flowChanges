...
for main_folder_num = 1:length(paths)
    ...
    tmp = dir([path 'results_111']);
    tmp = tmp(3).name;
    load([path 'results_111/' tmp '/results.mat'],'opt')
    
    
    path_load = [path 'results_111'];
    path_save = [path 'results_222'];
    ...
    for fileNum = 1:height(opt.info)
        ...
        stats_frame1_all = data.stats_frame1_all;
    ...
    %             tau_signal = tau_signal_all(use);
        ...
        save([ path_save '/' info.folder{fileNum} '/signals.mat'],'gamma_signals','tau_signals','times','polynoms',...
            'gamma_signals_orig','tau_signals_orig','times_orig','stats_frame1_all')
