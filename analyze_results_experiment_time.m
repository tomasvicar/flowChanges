...
for file_num = 1:size(file_names,1)
    ...
    for cell_num = 1:size(use_table,2)
        ...
        resutls(ind).pulse_ind = pulse_ind(use_vector);
...
% 
save_name = 'puze_index_G';
y_label = 'Shear modulus (Pa)';
x_label = 'Time under shear stress (s)';
ylims = [0,500];
% ylims = [0,3500];
% xlims = [0 3000];
...
for ind = 1:length(resutls)
    ...
    use = mod(resutls(ind).pulse_ind,2)==0;
...
xticklabels(num2cell(40:40:40*17))
